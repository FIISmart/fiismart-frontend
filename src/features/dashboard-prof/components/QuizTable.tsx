      ...
      <div className="bg-edu-card border border-edu-border rounded-2xl overflow-hidden shadow-sm overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-sm text-edu-foreground">
            {quizzes.length === 0 ? (
              <tr>
                <td colSpan={6} className="py-8 text-center text-edu-muted-fg">
                  Nu ai niciun quiz creat momentan.
                </td>
              </tr>
            ) : (
              quizzes.map((quiz) => (
                <tr
                  key={quiz.quizId}
                  className="border-b border-edu-border last:border-0 hover:bg-edu-bg/30 transition"
                >
                  ...
                  <td
                    className={`py-4 px-6 font-semibold ${
                      quiz.avgScorePct >= 75 ? "text-emerald-500" : "text-amber-500"
                    }`}
                  >
                    {quiz.avgScorePct}%
                  </td>
                  ...
                  <td
                    className={`py-4 px-6 font-semibold ${
                      quiz.status === "active" ? "text-emerald-500" : "text-amber-500"
                    }`}
                  >
