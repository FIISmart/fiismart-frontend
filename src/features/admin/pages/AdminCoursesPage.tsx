...
import { adminGetCourses, adminUpdateCourse, adminDeleteCourse, type AdminCourseAPI } from "@/lib/api";
...
export default function AdminCoursesPage() {
  ...
  const handleSaveEdit = async () => {
    ...
    setEditLoading(true);
    try {
      const updated = await adminUpdateCourse(editCourse.id, {
  ...
  const toggleHidden = async (c: AdminCourseAPI) => {
    try {
      const updated = await adminUpdateCourse(c.id, { hidden: !c.hidden });
  ...
  const handleDelete = async () => {
    ...
    try {
      await adminDeleteCourse(deleteCourseId);
