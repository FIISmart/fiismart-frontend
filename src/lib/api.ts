...
export async function apiFetch<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (import.meta.env.DEV) {
      console.warn(`[API] Network error fetching ${path}, trying dev mocks...`, networkErr);
  ...
  if (!res.ok) {
    if (import.meta.env.DEV) {
      console.warn(`[API] Request to ${path} failed with status ${res.status}, trying dev mocks...`);
      const mocked = await tryDevMockRequest(path, options);
      if (mocked !== undefined) return mocked as T;
    }
