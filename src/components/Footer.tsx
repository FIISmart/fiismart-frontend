      ...
      <div className="section-container py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="flex gap-3">
              {['f', 'in', 'tw', 'yt'].map((s) => (
                <div key={s} className="w-8 h-8 bg-white/10 rounded-md flex items-center justify-center text-caption font-medium hover:bg-primary transition-colors cursor-pointer">{s}</div>
              ))}
            </div>
          </div>
          {[
            ...
            <div key={col.title}>
              ...
              <ul className="flex flex-col gap-2">
                {col.links.map((link) => (
                  <li key={link}><a href="#" className="font-body text-body-sm text-white/60 hover:text-white transition-colors">{link}</a></li>
                ))}
              </ul>
            </div>
          ))}
        ...
        <div className="border-t border-white/10 pt-6 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4 text-white/40 text-caption">
            <span className="flex items-center gap-1"><Mail size={12} /> contact@FIISmart.ro</span>
            <span className="flex items-center gap-1"><Phone size={12} /> +40 721 000 000</span>
            <span className="flex items-center gap-1"><MapPin size={12} /> Bucuresti, RO</span>
          </div>
        </div>
      </div>
    </footer>
