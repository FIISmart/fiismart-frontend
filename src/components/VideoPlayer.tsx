...
export default function VideoPlayer({
    ...
    const syncWithBackend = useCallback(async (currTime: number, dur: number) => {
        if (currTime <= 0 || dur <= 0) return;
        const watchedPercent = Math.floor((currTime / dur) * 100);
        try {
            await fetch(`http://localhost:8081/api/students/${studentId}/courses/${courseId}/lectures/${lectureId}/progress`, {
                method: 'PUT',
    ...
    if (!src) return <div className="w-full aspect-video bg-black rounded-xl animate-pulse"/>;
            ...
            <div className="bg-card p-5 border-t border-border">
                ...
                >
                    ...
                    {showMarkers && markers.map((m) => (
                        <div
                            key={m.id}
                            className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-white border-[3px] border-primary rounded-full cursor-pointer hover:scale-150 transition-transform shadow-md z-10"
                ...
                <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center gap-5 text-foreground">
                        ...
                        <span
                            className="font-bold text-sm tracking-wide bg-muted px-3 py-1.5 rounded-lg border border-border">
